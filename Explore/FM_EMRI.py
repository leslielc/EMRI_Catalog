...
PSD = get_sensitivity(freq, sens_fn = 'cornish_lisa_psd')#'cornish_lisa_psd') 'lisasens'
...
M_step = 2e-1   #1e-1 np.logspace(-0.5,0.8)
...
delta_space = np.logspace(-0.7,0.5,25)
# delta_space = np.logspace(-1.5,1.5,10)
...
for i in range(N_params): #[5]: #range(N_params)  
    for delta in [1]: #(delta_space):   ##Test for step length
        ...
        for j in range(diff_info[1],diff_info[2],-1):
            if j == 0:
                continue
            waveform_step = AAK_waveform_model(M + j*uni_mat[i,0]*M_step, mu + j*uni_mat[i,1]*mu_step, a + j*uni_mat[i,2]*a_step, p0 + j*uni_mat[i,3]*p0_step, 
                                            e0 + j*uni_mat[i,4]*e0_step, Y0 + j*uni_mat[i,5]*Y0_step, dist + j*uni_mat[i,6]*dist_step, 
                                                qS, phiS, qK, phiK,
                                                Phi_phi0=(Phi_phi0 + j*uni_mat[i,7]*Phi_phi0_step), Phi_theta0= (Phi_theta0 + j*uni_mat[i,8]*Phi_theta0_step), Phi_r0=(Phi_r0 + j*uni_mat[i,9]*Phi_r0_step), 
                                                mich=False, dt=delta_t, T=T)  # Generate h_plus and h_cross
